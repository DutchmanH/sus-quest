...
export async function POST(
  ...
  if (id === user.id) return NextResponse.json({ error: 'Kan eigen admin-status niet wijzigen' }, { status: 400 })
  ...
  if (isProtectedSuperAdminEmail(targetEmail)) {
    ...
    return NextResponse.json({ error: 'Protected super admin kan niet gedegradeerd worden' }, { status: 403 })
  }
  ...
  return NextResponse.json({ is_admin: newAdmin })
}
